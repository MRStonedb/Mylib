...
class Mysql_Query:
	def __init__(self,host,user,password,port):
		...
		self.port = int(port)
		self.conn = pymysql.connect(host=host,user=user,password=password,port=int(port), charset='utf8')

	def reconnect(self):
		self.conn = pymysql.connect(host=self.host,user=self.user,password=self.password,port=self.port, charset='utf8')
