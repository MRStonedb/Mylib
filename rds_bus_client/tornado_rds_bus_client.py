...
from lib.rds_bus_client.common import RdsListData, RdsData, OperationType
...
class TornadoRdsBusClient(object):
    ...
    def __init__(self, url: str, logger, request_timeout: int = None, database: str = None):
        self._logger = logger
    ...
    @gen.coroutine
    def query(self, key: str, parameter: dict, order_by: list = None, page_no: int = None, per_page: int = None,
        ...
        if execute:
            response = yield self._send(operation=OperationType.QUERY, argument=argument)
    ...
    @gen.coroutine
    def insert(self, key: str, parameter: dict, database: str = None, execute: bool = True):
        """
        新增接口
        :param database: RDS-BUS的数据库类名
        :param key: 数据库类所持有的语句实例名
        ...
        if execute:
            response = yield self._send(operation=OperationType.INSERT, argument=argument)
            result = RdsData(response)
        else:
            result = argument
        return result
    ...
    @gen.coroutine
    def update(self, key: str, parameter: dict, database: str=None, execute: bool = True):
        """
        更新接口
        :param database: RDS-BUS的数据库类名
        :param key: 数据库类所持有的语句实例名
        ...
        argument = self.pack(database=_database, key=key, parameter=parameter)
        if execute:
            response = yield self._send(operation=OperationType.UPDATE, argument=argument)
            result = RdsData(response)
        else:
            result = argument
        return result
    ...
    @gen.coroutine
    def delete(self, key: str, parameter: dict, database: str = None, execute: bool = False):
        """
        删除接口
        :param database: RDS-BUS的数据库类名
        :param key: 数据库类所持有的语句实例名
        ...
        argument = self.pack(database=_database, key=key, parameter=parameter)
        if execute:
            response = yield self._send(operation=OperationType.DELETE, argument=argument)
            result = RdsData(response)
        else:
            result = argument
        return result
    ...
    def transaction(self, data: list, database: str = None):
        ...
        """
        _database = database or self._database
        result = yield self._send(operation=OperationType.TRANSACTION,
